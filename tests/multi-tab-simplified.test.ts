describe('Multi-tab grayscale functionality', () => {
  ...
  const mockChrome = {
    tabs: {
      query: jest.fn().mockImplementation(async (queryInfo: any) => {
  ...
  const applyGrayscaleToTab = async (tabId: number, isGrayscale: boolean) => {
    mockTabStates[`tab_${tabId}`] = { isGrayscale };
    // In the real implementation, this would update UI and apply CSS
  };
  
  const applyGrayscaleToAllTabs = async (isGrayscale: boolean) => {
  ...
  const toggleGrayscaleForTab = async (tabId: number) => {
